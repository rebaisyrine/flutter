...
class _AddNoteModalState extends State<AddNoteModal> {
  ...
  Future<void> _handleSave() async {
    ...
    try {
      ...
      final dynamic maybeData = (newNote is Map<String, dynamic>)
          ? newNote
          : (newNote as dynamic).data ?? <String, dynamic>{};
      final Map<String, dynamic> noteMap =
          Map<String, dynamic>.from(maybeData as Map);
      // If Appwrite uses $id, expose it as id
      if (noteMap.containsKey(r'$id') && !noteMap.containsKey('id')) {
        noteMap['id'] = noteMap[r'$id'];
      }
      widget.onNoteAdded(noteMap);
